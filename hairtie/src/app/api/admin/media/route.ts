...
export async function GET(request: Request) {
  ...
  const all = listMedia({
    ...
    folder: searchParams.get("folder")?.trim() || undefined,
  });

  return NextResponse.json({
    assets: all.slice(skip, skip + take),
    total: all.length,
...
export async function POST(request: Request) {
  ...
  if (files.length > 20) {
    return NextResponse.json({ message: "Please upload up to 20 images at a time." }, { status: 400 });
  }
...
export async function DELETE(request: Request) {
  ...
  // image still referenced by a product or a published page does not break.
  removeMedia(id);
